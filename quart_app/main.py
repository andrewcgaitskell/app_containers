...
# Register the Home Assistant blueprint
...
async def startup():
    ...
    # create a single aiohttp session to be reused by blueprints/helpers
    import aiohttp
    app.http_session = aiohttp.ClientSession()
    logging.info("MQTT listener started and HTTP session created.")
...
async def cleanup():
    ...
    if getattr(app, "http_session", None):
...
if __name__ == "__main__":
    # No HASS_TOKEN check here; blueprint will raise a helpful error if missing when called.
